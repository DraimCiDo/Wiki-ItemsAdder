...
### Получение пользовательского элемента любого типа (блок, предмет, шляпа, еда...) по id или namespace:id
...
{
    //не найден пользовательский предмет с таким идентификатором
}
```

### Проверка существования пользовательского элемента
...
или вы также можете использовать старый метод, который ничего не выделяет.

```java
ItemsAdder.isCustomItem("your_item");
```

### Получение данных пользовательского элемента из внутриигрового ItemStack
